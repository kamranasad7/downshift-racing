...
CORRECTION (final review caught): activeInputHandler is STILL 2 (Both), NOT 0. My execute_code SerializedObject write reported 2->0 but did not persist; commit 2f2e560's message "switch android to legacy input" is INACCURATE (the pause-hardening half of it IS real and correct). Both is confirmed SAFE by final review on Windows + Android (legacy Input.* + StandaloneInputModule both work; only value 1=New-only would break). Android "Both" warning is a perf advisory, not breakage. DO NOT strip com.unity.inputsystem while value is 2. Proper legacy-only switch (value 0 + editor restart + remove inputactions asset) left as an explicit optional step for the user.
Final review verdict: READY for morning playtest + phone sideload, zero blockers. Only correction is this input-handler record. minSdk is actually 25 (not 24), targetSdk auto - both fine.
